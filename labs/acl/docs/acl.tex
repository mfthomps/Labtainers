...
Alice creates a new file in the {\tt /shared\_files/alice} directory,
that new file will be readable by Bob, but not by users other than
Bob and Alice.  Consider doing this systematically:
\begin{itemize}
\item Create a file in {\tt /shared\_files/alice} and check its permissions
\item Set the default ACL on the alice directory to permit bob to read newly created files. 
\item Create another new file in {\tt /shared\_files/alice} and check its permissions.  Are they what you expect?
