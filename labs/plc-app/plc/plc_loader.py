...
water_level = ctypes.c_int.from_buffer(buf, 0)
...
while True:
    ...
    if rbytes is None:
        ...
        if parts[0] == 'load':
            ...
            if not status:
                status_string = 'PLC not running'
            else:
                status_string = 'PLC running \nWater level: %d\nParams: %s' % (water_level.value, config_params)
            sendData(connection, 'status:%s' % status_string)    
