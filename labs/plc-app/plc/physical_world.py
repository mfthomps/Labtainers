...
def logWaterLevel(water_level, reset_counter, timestamp):
    retval = False
    global log_20
    global log_30
    global count_resets
    ...
    if (now - timestamp) > 30:
        if log_30 > 0:
            logging.debug('no heartbeat now is %d, last was %d' % (timestamp, now))
        log_30 = 0
        log_20 = 0
...
def main(screen):
    ...
    timestamp = ctypes.c_int.from_buffer(buf, 16)
    ...
    while True:
         if not lab_success:
             lab_success = logWaterLevel(water_level.value, reset_counter.value, timestamp.value)
