...
\begin{figure}[!htb]
\centering
\includegraphics*[viewport=50 120 750 520, width=0.9\textwidth,natwidth=621,natheight=403]{Figs/topology.jpg}
...
%The attacker needs to run {\tt Netwag} or {\tt Netwox} as the {\tt root} user. 
...
Note: the {\tt ANSWER SECTION} contains the DNS mapping. You can notice that
...
%\subsection{Use nonamed in Minix}
%
%If we use Minix under VMWare with NAT enabled, the DNS server, nonamed, is
%built in. With nonamed, the DNS request to Minix will be routed to an external
%DNS server, i.e. the DHCP server or the IP of host machine. We can check the IP
%address of the external DNS server in Minix by \texttt{dhcpd -q}
...
%Always use wireshark to monitor what's really going on while doing the 
%following attacks.
...
\begin{figure}[!htb]
\centering
\includegraphics*[ width=0.9\textwidth,natwidth=621,natheight=403]{Figs/DNS_Local_flow1.pdf}
...
\begin{figure}[!htb]
\centering
\includegraphics*[ width=0.9\textwidth,natwidth=621,natheight=403]{Figs/DNS_Local_flow2.pdf}
\caption{DNS cache posioning flow} 
\label{fig:flow2}
\end{figure}


We can use the same tool (Netwox 105) for this attack. Before attacking, 
...
This spoofed IP address is usually a root DNS server (this is usually
the first place that a DNS server will ask if it cannot resolve a name),
and obviously the root DNS server is not on the same LAN. Therefore,
