...
\item {\bf Docker (Community Edition)}
    \begin {itemize}
    \item {For full and convenient installation of Docker and setting of Docker privileges, run {\tt install-docker-ubuntu.sh} in {\tt setup\_scripts}, assuming you are developing in Ubuntu. 
    ...
    \item {\bf NOTE:} These install-docker scripts include the installation of other packages outside Docker that are necessary for  
    \end {itemize}
\end {itemize}
...
if [[ ":$PATH:" != *":./bin:"* ]]; then
  ...
  export PATH="${PATH}:./bin:$LABTAINER_DIR/scripts/designer/bin"
    ...
    \end{lstlisting}} 
    \item cd into \$LABTAINER\_DIR/setup\_cripts:
	    \begin{itemize}
