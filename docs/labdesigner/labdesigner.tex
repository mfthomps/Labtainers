...
\item Create a ``host-only'' network and allocate it to each of the student VMs (typically as an additional network interface).
Enable a DHCP server for the VMs on the host-only network, or otherwise configure them so that each is allocated an IP address in the
client subnet.
