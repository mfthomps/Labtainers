...
\begin{itemize}
\item \textbf{window} $<$text$>$ -- Selects the window having a title that contains
the given text.
\item \textbf{type\_line} $<$text$>$ -- Types the given text.
\item \textbf{key} $<$keysym$>$ -- Performs a keypress for the given X11 keysim, see
\url{http://xahlee.info/linux/linux\_show\_keycode\_keysym.html}
...
\item \textbf{replace\_file} $<$source file$>$ $<$dest file$>$ -- Copies content of a source
file on the Linux host to a file on the currently selected container.  Uses a cheezy vi
scheme that assumes you have selected the proper window.
\item \textbf{add\_file} $<$source file$>$ $<$dest file$>$ -- Like {\tt replace\_file}, but
will append text from the source to the end of the destination.
