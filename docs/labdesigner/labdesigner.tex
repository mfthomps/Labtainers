...
DHCP server on this network.   The network adaptor itself needs to be placed in primiscous mode on the VMM, and on the 
...
container is derived from the MAC address of the Linux VM host-only network interface. 
...
``+CLONE\_MAC'' option, e.g., 
\begin{verbatim}
    LAN 192.168.1.10+CLONE_MAC
\end{verbatim}
\noindent  When run in single user mode, the +CLONE\_MAC suffix is ignored.  When run
