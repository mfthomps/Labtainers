...
See the {\tt routing\_basics2} lab for examples.
...
{\tt insert thumb drive} from a component menu.
...
\section {Remote control of Labtainer exercises}
This section describes a feature intended for use within structured environments in which one or more students are performing 
a lab exercise under supervision of an instructor.  This does not apply to environments in which students 
individualy run Labtainers on dedicated computers at their own pace. 

Consider an environment in which multiple Labtainer VMs (or custom-built VMs containing Labtainers) run on one or 
more servers that are networked together.  
Students interact individually with their allocated
VM using a tool such as VMWare Horizon, which presents the student with the Linux desktop of their allocated VM.
We assume that something within this infrastructure allows remote network access by an instructor to each VM, e.g.,
via port forwarding.  An alternate supported environment is one in which each student has a dedicated computer upon which a 
Labtainer VM resides.

Labtainer remote access functions allow instructors to query and change the state of the Labtainers exercise 
...
\subsection{Client and server setup}
