...
\item whether the file contains a given string
...
Consider the labs/formatstring/instr\_config/results.config file.  The third non-comment line
defines a result having the symbolic name ``origsecret1value''.  This result is
  ...
  Where: 
    ...
    <type> - must be one of the following:
         ...
         time_before - Both goal1 and goal2 must be goal_ids from previous 
                         *matchany, is_true, or is_false* goal types.  
                         Evaluates to TRUE if any TRUE goal1 has a timestamp 
                         that is before than any TRUE goal2
         time_during - Both goal1 and goal2 must be goal_ids from previous 
                         *matchany, is_true, or is_false* goal types.  
                         Timestamps include a start and end time, reflecting 
                         when the program starts and when it 
         ...
         value       - The goal value is assigned the given resulttag value from
                         ...
                         NOTE: use is_true for boolen expressions within goals where
                         you are evaluating values having a common timestamp.
...
That home.tar also includes a {\tt ~/.cache/dconf} directory to avoid Firefox error
messages.
Use an {\tt external-manifest} file (per section~\ref{manifest}) to link to that 
tar to avoid firefox starting with its welcome pages and privacy statements.
The labtainer.firefox image includes a customized /usr/bin/firefox that starts the
browswer in a new instance so it does share existing browsers. The {\tt about:config}
...
In short, put this before the apt-get that installs wireshark:
    RUN echo 'wireshark-common wireshark-common/install-setuid \
