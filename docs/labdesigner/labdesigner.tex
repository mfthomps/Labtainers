...
\item MACVLAN\_EXT [device] -- Optional, causes the Docker network driver to 
create and use a macvlan tied to the given ethernet
interface.  The network device is expected to be on a ``host-only'' VM network.  The VMM should disable the
DHCP server on this network.   
The network adaptor itself needs to be placed in primiscous mode on the VMM, and on the 
...
is started as a multi-user lab.
...
\item {\tt CLONE\_MAC} Only takes effect if the lab is started in multi-user mode.  When started with the 
{\tt --workstation} switch, this directs the system to generate a MAC address whose last four bytes match 
those of the host network interface.  When stated as a multi-user lab with all containers on one VM, e.g., the 
{\tt --clone\_count} switch, then the allocated IP address is incremeted by one less than the clone instance number.
\end{itemize}
...
same lab.  Labtainers does not yet include complete examples labs intended for multi-user environments, however it does
offer support for such environments.  For simplicity, we assume that multi-user labs will take one of two
different forms:  
\begin{enumerate}
\item All containers are on one VM and students SSH into containers from different VMs in the same farm, or 
\item Each student is allocated a VM on a VM farm, some other VM on that farm will host the shared components, and all of
these VMs have Labtainers installed.
\end{enumerate}
...
allocated to the student.  The ssh command may include the ``-X'' option to permit X11 forwarding, thus allowing students to
run GUI-based applications on their workstation containers.

\begin{itemize}
\item Pick a ``client subnet'' that will be used by student to 
SSH into their container workstations.

\item Create a ``host-only'' network on the VMM and disable the DHCP server on the VMM.
Do this before assigning the network interface to VM (VirtualBox  ignores dhcp setting changes
after assignment?).
Student VMs will get their host-only
network addresses from the Labtainer dchp service.

and allocate it to each of the student VMs (typically as an additional network interface).

\item Allocate the host-only network to the Labtainer VM, and set it in promiscuous mode, and disable the network at the IPv4 level,
...
\item Allocate a ``workstation'' container for each student.
...
\item Define a MACVLAN network on the client subnet in the start.config file.  Assign network addresses to the
student workstation container using the {\tt +CLONE} syntax described in \ref{start config}.  

\item Include and SSH server in the workstation container, e.g., by deriving it from the labtainer.network base.
...
\item Password management. Assuming you'd like to allocate each student a unique (insecure) password for purposes of
...
\noindent And, this in the labs/[your lab]/config/parameter.config
\begin{verbatim}
...
individual VM,
and the student has access to that VM, e.g., via ssh or a vSphere client.  
The remaining containers, e.g., vulnerable servers, all run on a single VM,
which we refer to herein as the ``server VM''.
All of these VMs have Labtainers installed.

\begin{itemize}
\item Define a host-only network and allocate it to each Labtainers VM.  Be sure
...
\item Define a MACVLAN network on the client subnet in the start.config file.
...
for the student container network connection.   
...
file.  Edit the dhcp container's {\tt \_system/etc/dnsmasq.conf} file to include the
range of DHCP addresses you wish to allocate to the clients, e.g., 
\begin{verbatim}
    dhcp-range=192.168.1.10, 192.168.1.99, 12h
\end{verbatim}


\item Enable dhcp on the clients by installing ics-dhcp-client
(via the Docerfile), and putting this in the workstation /etc/rc.local:
\begin{verbatim}
...
\item DISABLE dhcp server on the VMM host-only network.

\end{itemize}
Start the lab on the server VM using start.py and the --server (-s) switch.  The students then
start the lab on their individual VMs using start.py without the server switch, which will cause
the student VM to only start the container identified as the ``CLIENT'' in the start.config file.

The dhcp-test lab is an example.
