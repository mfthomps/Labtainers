...
Ubuntu package sources, which may differ from those in your host /etc/apt/sources.list file.  You
can force the Labtainer to use your host sources.list by including
\begin{verbatim}
COPY $labdir/dockerfiles/tmp/sources.list /etc/apt/
\end{verbatim}
\noindent in your Dockerfile prior to the installation of packages.  The Labatainer labs from NPS 
generally have sources.list entries that reflect what was working at NPS at the time of the build.
