...
endpoint device, which interacts with an Ethernet network interface.
The following assumes your VM has a virtual bridge Ethernet interface named {\tt virbr0}, with IP address {\tt 192.168.122.1/24}.
...
address on the {virbr0} subnet, e.g., {\tt 192.168.122.100}.  Then, 
...
\item Use port forwarding as described earlier to map host ports to ports on the VM.
\end{itemize}
...
private.  (TBD: The connected component and the cloud device itself should be hidden from the student performing the lab.)
