...
Text included within the directives will be displayed to students based on the values
of their associated goals at the time {\tt checkwork} was run.

\subsection {Current state artifacts}
Results and goals used for current state assessment are typically derived from artifacts generated by {\tt prestop} scripts 
described in \ref{prestop}.  The system uses the most recent timestamp found for any files named by current state
results, i.e., those with the {\tt cw\_} prefix.  The designer can name any file for a current state result.
