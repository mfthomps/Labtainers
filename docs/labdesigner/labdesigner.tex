...
Dockerfile.  A default Labtainer-specific Dockerfile is placed in the new lab's ``dockerfiles'' 
directory when the new lab is created.  And additional Dockerfiles are added when the
{\tt new\_lab\_setup.sh -a} script is run.  We use standard Docker file syntax, which is described at 
...
when a new lab (or additional container) is created.  
...
e.g., {\tt httpd.service} would generate timestamped artifact files whenever http was start (or restarted)
using systemctl or /etc/init.d.  See section \ref{time delimeter services}
...
\subsection{Assessing student results}
...
\subsubsection{Compare value of field from selected line in an artifact file}
...
Then, if the lab result.config were:
\begin{verbatim}
