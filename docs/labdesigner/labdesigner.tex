...
See the {\tt routing\_basics2} for examples.
\end{itemize}
...
a ``ignorelocal'' file in that same directory.  These should not include path information, just the program name.
...
delineated by timestamp ranges described in section \ref{results.config}.  In those cases 
...
The goals.config file contains directives, each of which assigns a value to symbolic name referred
...
generated from the student email address as described in section \ref{parameterizing}.
A boolean goal\_id value is generated for each referenced result's timestamp.
...
\subsection{Automated assessment usage and examples}
...
\subsubsection{Get field from selected line in an artifact file}
...
\subsubsection{time\_delimeter file name qualifier} These are intended for use with 
\label{time delimeter}
log files having timestamped entries.  The {\tt time\_delimiter} identifies some
...
delimited by the start times of the monitored program.  The intended use is to
...
to confirm that some set of expected stdout from nmap all occurred within a single
configuration of iptables, delimited by the running of rc.local.  In other words,
the the student cannot succeed by altering iptables between invocations of nmap on
different components.  In this example, the {\tt file\_path} would name the iptables
log, and the {\tt time\_delimiter} would name rc.local.  If the lab result.config were:
\begin{verbatim}
...
\noindent The {\tt \_iplog} would then have up to N+1 timestamped results,
where N is the quantity of times that rc.local was executed. The first possible
timestamp would have a starting time of zero and and ending time of the very first
...
{\tt \_iplog} results.  The {\tt remote\_correct} boolean expression could then be read as: ``Was there any
single iptables configuration in which:
\begin{itemize}
