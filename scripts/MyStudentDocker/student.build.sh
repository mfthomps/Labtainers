...
ONEWAYHASH_DIR=${ORIG_PWD}/onewayhash/
ONEWAYHASH_TAR=${ORIG_PWD}/onewayhash.student.tar.gz

cp -r bin $PARAMLABS_DIR/.local/
cp -r bin $BUFOVERFLOW_DIR/.local/
cp -r bin $ONEWAYHASH_DIR/.local/
cd $PARAMLABS_DIR
tar -zcvf $PARAMLABS_TAR .local *
cd $BUFOVERFLOW_DIR
tar -zcvf $BUFOVERFLOW_TAR .local *
cd $ONEWAYHASH_DIR
tar -zcvf $ONEWAYHASH_TAR .local *
...
docker build -f ${ORIG_PWD}/Dockerfile.onewayhash.student -t onewayhash:student .
