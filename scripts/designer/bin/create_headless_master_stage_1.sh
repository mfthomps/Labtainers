...
source ./set_reg.sh
if [[ "$1" != -f ]]; then
   echo "This will build the labtainer master controller image (STAGE 1).  "
...
cp -a $LABTAINER_DIR/headless-lite/Dockerfile.labtainer.master.stage.1 workspace_master/Dockerfile.labtainer.headless.stage.1
